...
BEGIN
  ...
  IF v_searches = 0 AND v_audited = 0 THEN
    v_score := 0;
    v_status := 'unknown';
  ELSIF v_audited = 0 THEN
    v_score := CASE
      WHEN v_searches > 0 AND v_zero::float / GREATEST(v_searches,1) < 0.1 THEN 85
      WHEN v_searches > 0 AND v_zero::float / GREATEST(v_searches,1) < 0.3 THEN 65
      ELSE 40
    END;
