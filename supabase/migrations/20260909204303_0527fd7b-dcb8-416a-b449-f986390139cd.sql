...
BEGIN
  ...
  city_norm := unaccent(lower(trim(p_city)));
  ...
  WHERE c.situacao_cadastral = 'ATIVA'
    ...
    AND unaccent(lower(c.cidade)) = city_norm
...
REVOKE EXECUTE ON FUNCTION public.search_companies_exact_city(text, text, text[], text, integer, integer) FROM anon, public;
GRANT EXECUTE ON FUNCTION public.search_companies_exact_city(text, text, text[], text, integer, integer) TO authenticated, service_role;