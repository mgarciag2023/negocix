...
ALTER TABLE public.segment_stats
  ...
  ADD COLUMN IF NOT EXISTS total_leads_returned bigint NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS zero_result_searches integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS avg_relevance integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS suspicious_pct integer NOT NULL DEFAULT 0;

CREATE OR REPLACE FUNCTION public.refresh_segment_stats_from_audit(
  p_segment_key text,
  p_segment_label text,
  p_terms_count integer DEFAULT NULL
) RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE created_at > now() - interval '30 days'
    AND search_type = 'leads'
    AND lower(coalesce(search_config->>'segment','')) ILIKE p_segment_label || '%';
  ...
  IF v_searches = 0 THEN
    v_score := 0;
    v_status := 'unknown';
  ELSE
    v_score := LEAST(100, GREATEST(0,
      ROUND(v_avg_rel * 0.5)
      + ROUND((100 - v_susp_pct) * 0.3)
      + CASE WHEN v_zero::float / GREATEST(v_searches,1) < 0.1 THEN 20
  ...
  -- alertas
  IF v_searches > 0 AND v_zero::float / v_searches >= 0.5 THEN
    v_alerts := v_alerts || jsonb_build_object(
      ...
      'recommended_action','Adicionar sinônimos e variações regionais; revisar termos cadastrados.');
  ELSIF v_searches > 0 AND v_zero::float / v_searches >= 0.25 THEN
    v_alerts := v_alerts || jsonb_build_object(
      ...
      'recommended_action','Revisar filtros (estado/cidade) e ampliar termos.');
  END IF;

  IF v_audited >= 20 AND v_susp_pct >= 40 THEN
    v_alerts := v_alerts || jsonb_build_object(
      'alert_type','low_relevance','priority','high',
      'reason', v_susp_pct || '% dos leads recentes são suspeitos (não casam com nenhum termo).',
      'impact','Usuários recebem resultados fora do segmento esperado.',
      'recommended_action','Refinar termos: remover genéricos e adicionar específicos do nicho.');
  END IF;
  ...
  -- refresh alertas: remove ativos e insere novos
...
GRANT EXECUTE ON FUNCTION public.refresh_segment_stats_from_audit(text, text, integer) TO authenticated, service_role;
